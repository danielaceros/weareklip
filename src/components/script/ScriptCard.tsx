...
export function ScriptCard({ script, onView, onDelete }: ScriptCardProps) {
  ...
  const title = useMemo(
    () =>
      script.isAI
        ? script.description || t("scriptsCard.untitled")
        : script.videoTitle || t("scriptsCard.replicatedVideo"),
    [script.isAI, script.description, script.videoTitle, t]
  );
      ...
      <div className="flex flex-col h-full p-3 space-y-2">
        ...
        {script.rating !== undefined ? (
          <div
            ...
            aria-label={t("scriptsCard.ratingAria", { rating: script.rating ?? 0 })}
          >
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
          ...
          <p className="text-xs text-muted-foreground">{t("scriptsCard.unrated")}</p>
        )}

        {/* 3) Contenido / preview */}
        <div className="text-xs text-muted-foreground leading-relaxed">
          {script.isAI ? (
            <p className="line-clamp-4">{script.script || t("scriptsCard.noContent")}</p>
          ) : (
            <>
              {script.videoThumbnail ? (
                <div className="relative mb-2 w-full overflow-hidden rounded-md border border-border/60 bg-muted/30 aspect-video">
                  <Image
                    ...
                    alt={script.videoTitle || t("scriptsCard.videoThumbnailAlt")}
                    fill
                ...
                <div className="w-full h-24 flex items-center justify-center text-xs text-muted-foreground border rounded bg-muted/20">
                  {t("scriptsCard.previewUnavailable")}
                </div>
              )}
              <p className="line-clamp-3">
                {script.videoDescription || script.script || t("scriptsCard.noContent")}
              </p>
            </>
          )}
        </div>
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-2">
          <Button
            ...
            onClick={onView}
            aria-label={t("scriptsCard.viewAria", { title })}
          >
            {t("scriptsCard.view")}
          </Button>
          ...
          <div className="flex items-center gap-1">
            ...
            />
            <button
              aria-label={t("scriptsCard.deleteAria", { title })}
              onClick={onDelete}
              ...
              title={t("scriptsCard.deleteTitle")}
            >
