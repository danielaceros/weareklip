...
import { collection, orderBy, query, where, updateDoc, doc, onSnapshot } from "firebase/firestore"
...
const mensajesAmigables: Record<string, (message: string) => string> = {
  guion: (message: string) => {
    if (message.includes("aprobó")) return "✅ Aprobaste un guión"
    if (message.includes("editó")) return "✏️ Editaste un guión"
    if (message.includes("creó")) return "📝 Se creó un nuevo guión para ti"
    if (message.includes("solicitó cambios")) {
      const partes = message.split("solicitó cambios en guión:")
      const nombre = partes[1]?.trim() || ""
      return `💬 Solicitaste cambios en guión: ${nombre}`
    }
    return message
  },
  video: (message: string) => {
    if (message.includes("aprobó video")) {
      const partes = message.split("aprobó video:")
      const nombre = partes[1]?.trim() || ""
      return `✅ Aprobaste el video: ${nombre}`
    }
    if (message.includes("subió")) return "🎥 Se subió un nuevo video para ti"
    if (message.includes("editado")) return "✏️ Editaste un video"
    if (message.includes("solicitó cambios en video")) {
      const partes = message.split("solicitó cambios en video:")
      const nombre = partes[1]?.trim() || ""
      return `💬 Solicitaste cambios en video: ${nombre}`
    }
    return message
  },
